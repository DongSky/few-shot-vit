...
class ConvPool(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, pad_type='', stride=1):
        ...
        #self.max_pool_proj = nn.Sequential(nn.MaxPool2d(3, 1, 1),
        #                            nn.Conv2d(in_channels, in_channels // 4, 1, 1, 0),
        #                            nn.ReLU(inplace=True),
        #                            nn.Conv2d(in_channels // 4, out_channels, 1, 1, 0),)
        self.conv = create_conv2d(in_channels, out_channels, kernel_size=3, padding=pad_type, bias=True)
        #self.weight = nn.Sequential(nn.Conv2d(in_channels, in_channels // 4, 1, 1, 0),
        #                            nn.ReLU(inplace=True),
        #                            nn.Conv2d(in_channels // 4, out_channels, 1, 1, 0),
        #                        )
        self.norm = norm_layer(out_channels)
    ...
    def forward(self, x):
        ...
        #x = self.adapool(x)
        x = self.conv(x)
        #x2 = self.max_pool_proj(x)
        #weight = torch.sigmoid(self.weight(x.mean(3, keepdim=True).mean(2, keepdim=True)))
...
class Nest(nn.Module):
    ...
    def __init__(self, img_size=224, in_chans=3, patch_size=4, num_levels=3, embed_dims=(128, 256, 512),
                 ...
                 pad_type='', weight_init='', global_pool='avg', pos_embed_dim=32):
        """
        Args:
...
@register('nest_micro_resembed_80')
...
if __name__ == "__main__":
    model = nest_micro_80(num_classes=64)
    # model = BMLTest()
