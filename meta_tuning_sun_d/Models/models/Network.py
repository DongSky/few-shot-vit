...
from .nest import nest_micro_80, nest_nano_80
from .visformer import visformer_small_80
...
class DeepEMD(nn.Module):
    ...
    def __init__(self, args, mode='meta'):
        ...
        self.mode = mode
        self.args = args
        if args.backbone == "nest_nano_80":
            self.encoder = nest_nano_80()
            self.dim = 384
        elif args.backbone == "nest_micro_80":
            self.encoder = nest_micro_80()
            self.dim = 512
        elif args.backbone == 'visformer':
            self.encoder = visformer_small_80()
            self.dim = 512
        if self.mode == 'pre_train':
            self.fc = nn.Linear(self.dim, self.args.num_class)
    ...
    def get_sfc(self, support):
        ...
        # init the proto
        SFC = support.view(self.args.shot, -1, self.dim, support.shape[-2], support.shape[-1]).mean(dim=0).clone().detach()
