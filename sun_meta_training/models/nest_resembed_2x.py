...
class ConvPool(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, pad_type='', stride=1, downsample=True):
        super().__init__()
        ...
        if downsample == True:
            ...
            self.pool = create_pool2d('max', kernel_size=3, stride=1, padding=pad_type)
...
class NestLevel(nn.Module):
    ...
    def __init__(
            ...
            norm_layer=None, act_layer=None, pad_type='', downsample=True):
        super().__init__()
        self.block_size = block_size
        self.pos_embed = nn.Parameter(torch.zeros(1, num_blocks, seq_length, embed_dim))

        if prev_embed_dim is not None:
            self.pool = ConvPool(prev_embed_dim, embed_dim, norm_layer=norm_layer, pad_type=pad_type, downsample=downsample)
        else:
    ...
    def __init__(self, img_size=224, in_chans=3, patch_size=4, num_levels=3, embed_dims=(128, 256, 512),
        ...
        for i in range(len(self.num_blocks)):
            ...
            if i == len(self.num_blocks) - 1:
                ...
                self.num_blocks[i], self.block_size*2, self.seq_length*4, num_heads[i], depths[i], dim, prev_dim,
