...
from timm.models.layers import PatchEmbed, Mlp, DropPath, to_2tuple, trunc_normal_, to_ntuple, get_act_layer
...
model_cfgs = dict(
    ...
    levit_384=dict(
        embed_dim=(256, 384, 512), key_dim=32, num_heads=(4, 6, 8), depth=(2, 3, 2)),
    levit_384s=dict(
        embed_dim=(384, 512, 768), key_dim=32, num_heads=(6, 9, 12), depth=(2, 2, 2)),
)
...
# @register('levit_128_80')
def levit_128_80(pretrained=False, use_conv=False, **kwargs):
    return levit_128(pretrained=pretrained, use_conv=use_conv, img_size=80)
    # return create_levit(
    #     'levit_128', pretrained=pretrained, use_conv=use_conv, **kwargs)
...
# def levit_384s(pretrained=False, use_conv=False, **kwargs):
#     return create_levit(
#         'levit_384s', pretrained=pretrained, use_conv=use_conv, **kwargs)
@register("levit_micro_80")
def levit_micro_80(pretrained=False, use_conv=False, **kwargs):
    return levit_384(pretrained=pretrained, use_conv=use_conv, img_size=80)
...
class Levit(nn.Module):
    ...
    def __init__(
        ...
        self.patch_embed = ConvBlock(in_chans, 128, embed_dim[0])#hybrid_backbone or stem_b16(in_chans, embed_dim[0], activation=act_layer)
    ...
    def forward_features(self, x):
        ...
        x1 = x.mean((-2, -1)) if self.use_conv else x.mean(1)
        return feat, x1
