...
class ConvBlock(nn.Module):
    ...
    def __init__(self, inplanes, hidden_planes, planes):
        ...
        self.conv1 = nn.Conv2d(inplanes, hidden_planes, 7, stride=2, padding=3, bias=False)
        ...
        self.downsample = nn.Sequential(conv3x3(inplanes, planes),norm_layer(planes), nn.MaxPool2d(2))
        # self.last = last
...
class AdaptivePool(nn.Module):
    def __init__(self, embed_dim):
        super().__init__()
        self.avg = nn.Conv2d(embed_dim, embed_dim, 3, 1, 1, groups=embed_dim) #nn.AvgPool2d(3, 1, 1)
        self.max = nn.MaxPool2d(3, 1, 1)
        self.weight = nn.Parameter(torch.zeros(1, embed_dim, 1, 1))
    def forward(self, x):
        #B, H, W, C = x.size()
        #x = x.permute(0, 3, 1, 2)
        x = self.avg(x) * (1 - torch.sigmoid(self.weight)) + self.max(x) * torch.sigmoid(self.weight)
...
class ConvPool(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, pad_type='', stride=1):
        super().__init__()
        #self.adapool = AdaptivePool(in_channels)
...
class Nest(nn.Module):
    ...
    def __init__(self, img_size=224, in_chans=3, patch_size=4, num_levels=3, embed_dims=(128, 256, 512),
        ...
        # Patch embedding
        self.patch_embed = ConvBlock(inplanes=in_chans, hidden_planes=64, planes=embed_dims[0])
...
@register('nest_micro_resembed_ada_80')
...
class Nest_Share(nn.Module):
    ...
    def __init__(self, img_size=224, in_chans=3, patch_size=4, num_levels=3, embed_dims=(128, 256, 512),
        ...
        # Number of blocks at each level
        self.num_blocks = (4 ** torch.arange(num_levels+1)).flip(0).tolist()[:2]
        assert (img_size // patch_size) % math.sqrt(self.num_blocks[0]) == 0, \
            'First level blocks don\'t fit evenly. Check `img_size`, `patch_size`, and `num_levels`'
    ...
    def forward(self, x):
        ...
        # if self.drop_rate > 0.:
        #     x1 = F.dropout(x1, p=self.drop_rate, training=self.training)
        # return x, x1#self.head(x)

class Nest_Indep(nn.Module):
    """ Nested Transformer (NesT)
    A PyTorch impl of : `Aggregating Nested Transformers`
        - https://arxiv.org/abs/2105.12723
    """

    def __init__(self, seq_length, block_size, prev_dim, img_size=224, in_chans=3, patch_size=4, num_levels=3, embed_dims=(128, 256, 512),
                 num_heads=(4, 8, 16), depths=(2, 2, 20), num_classes=1000, mlp_ratio=4., qkv_bias=True,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.5, norm_layer=None, act_layer=None,
                 pad_type='', weight_init='', global_pool='avg'):
        """
        ...
        # Number of blocks at each level
        self.num_blocks = [1] #(4 ** torch.arange(num_levels)).flip(0).tolist()
        assert (img_size // patch_size) % math.sqrt(self.num_blocks[0]) == 0, \
            'First level blocks don\'t fit evenly. Check `img_size`, `patch_size`, and `num_levels`'
        ...
        # Hint: (img_size // patch_size) gives number of patches along edge of image. sqrt(self.num_blocks[0]) is the
        #  number of blocks along edge of image
        self.block_size = block_size#int((img_size // patch_size) // math.sqrt(self.num_blocks[0]))

        # Patch embedding
        # self.patch_embed = PatchEmbed(
        #     img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dims[0], flatten=False)
        # self.num_patches = self.patch_embed.num_patches
        self.seq_length = seq_length#self.num_patches // self.num_blocks[0]
...
class BMLTest(nn.Module):
    def __init__(self, img_size=80, embed_dims=(128, 384, 512), num_heads=(4, 12, 16), depths=(2, 2, 2)):
        super().__init__()
        self.share = Nest_Share(img_size, embed_dims=embed_dims[:2], num_heads=num_heads[:2], depths=depths[:2], num_levels=2)
        self.global_view = Nest_Indep(seq_length=self.share.seq_length, block_size=self.share.block_size, prev_dim=embed_dims[1], embed_dims=embed_dims[2:], num_heads=num_heads[2:], depths=depths[2:], num_levels=1)
        self.local_view = Nest_Indep(seq_length=self.share.seq_length, block_size=self.share.block_size, prev_dim=embed_dims[1], embed_dims=embed_dims[2:], num_heads=num_heads[2:], depths=depths[2:], num_levels=1)
    def forward(self, x):
        x = self.share(x)
        x1 = self.global_view(x)
        x2 = self.local_view(x)
        return x1, x2
