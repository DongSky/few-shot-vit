...
class AdaptivePool(nn.Module):
    ...
    def forward(self, x):
        B, H, W, C = x.size()
        x = x.permute(0, 3, 1, 2)
        x = self.avg(x) #+ self.max(x) * torch.sigmoid(self.weight)
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.out_dim = self.num_features = int(embed_dim * 2 ** (self.num_layers - 1))
        self.mlp_ratio = mlp_ratio
        ...
        self.patch_embed = ConvBlock(inplanes=in_chans, hidden_planes=64, planes=embed_dim)
        self.grid_size = (img_size // patch_size, img_size // patch_size)
        self.num_patches = self.grid_size[0] * self.grid_size[1]
        num_patches = self.num_patches#self.patch_embed.num_patches
        self.patch_grid = self.grid_size#self.patch_embed.grid_size
    ...
    def forward_features(self, x):
        ...
        x = self.layers(x)
        x = self.norm(x)  # B L C
        x1 = self.avgpool(x.transpose(1, 2))  # B C 1
...
# @register('swin_nano_patch4_window5_80')
def swin_nano_patch4_window5_80(pretrained=False, **kwargs):
    """ Swin-nano @ 80x80, from scratch
    """
    model_kwargs = dict(img_size=96, 
        patch_size=4, window_size=6, embed_dim=64, depths=(1, 1, 1, 2), num_heads=(2, 4, 8, 16), **kwargs)
    return _create_swin_transformer('swin_tiny_patch4_window7_224', pretrained=pretrained, **model_kwargs)

@register('swin_micro_resembed_80')
def swin_micro_80(pretrained=False, **kwargs):
    """ Swin-nano @ 80x80, from scratch
    """
    model_kwargs = dict(img_size=80, 
        patch_size=4, window_size=5, embed_dim=160, depths=(1, 1, 2), num_heads=(4, 8, 16), **kwargs)
    return _create_swin_transformer('swin_tiny_patch4_window7_224', pretrained=pretrained, **model_kwargs)
@register('swin_micro_v2_resembed_ada_80')
def swin_micro_v2_80(pretrained=False, drop_path_rate=0.5, **kwargs):
    """ Swin-nano @ 80x80, from scratch
    """
    model_kwargs = dict(img_size=80,
        patch_size=4, window_size=5, embed_dim=144, depths=(2, 3, 2), num_heads=(4, 8, 16), drop_path_rate=drop_path_rate, **kwargs)
    return _create_swin_transformer('swin_tiny_patch4_window7_224', pretrained=pretrained, **model_kwargs)
# @register_model
...
if __name__ == "__main__":
    model = swin_micro_80(drop_path_rate=0.5)
    n_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(n_parameters / 1000 / 1000, "M params")
