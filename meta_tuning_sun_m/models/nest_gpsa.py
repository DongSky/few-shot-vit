...
class GPSA(nn.Module):
    ...
    def get_attention(self, x):
        ...
        q, k = qk[0], qk[1]
        pos_score = self.rel_indices.expand(B, -1, -1, -1, -1)
        pos_score = self.pos_proj(pos_score).permute(0, 4, 1, 2, 3)
        patch_score = (q @ k.transpose(-2, -1)) * self.scale
        patch_score = patch_score.softmax(dim=-1)
        pos_score = pos_score.softmax(dim=-1)

        gating = self.gating_param.view(1, -1, 1, 1, 1)
        # print(gating.size())
        # print(patch_score.size())
    ...
    def get_attention_map(self, x, return_map=False):
        ...
        dist = torch.einsum('tnm,htnm->h', (distances, attn_map)) / distances.size(0)
        if return_map:
    ...
    def get_rel_indices(self, num_patches: int) -> torch.Tensor:
        ...
        rel_indices = torch.zeros(1, 1, num_patches, num_patches, 3)
...
class TransformerLayer(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, use_gpsa=False):
        super().__init__()
...
class NestLevel(nn.Module):
    ...
    def __init__(
            ...
            norm_layer=None, act_layer=None, pad_type='', use_gpsa=False):
        super().__init__()
        ...
        self.transformer_encoder = nn.Sequential(*[
            TransformerLayer(
                ...
                norm_layer=norm_layer, act_layer=act_layer, use_gpsa=use_gpsa)
...
class Nest(nn.Module):
    ...
    def __init__(self, img_size=224, in_chans=3, patch_size=4, num_levels=3, embed_dims=(128, 256, 512),
        ...
        for i in range(len(self.num_blocks)):
            use_gpsa = True if i < 2 else False
            dim = embed_dims[i]
            levels.append(NestLevel(
                self.num_blocks[i], self.block_size, self.seq_length, num_heads[i], depths[i], dim, prev_dim,
                mlp_ratio, qkv_bias, drop_rate, attn_drop_rate, dp_rates[i], norm_layer, act_layer, pad_type=pad_type, use_gpsa=use_gpsa))
